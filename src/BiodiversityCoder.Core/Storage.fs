...
module Storage =
    ...
    let inline loadCacheFile< ^T> directory filename : Result< ^T, string > =
        if Directory.Exists directory then
            if File.Exists(Path.Combine(directory, filename)) then
                try
                    Path.Combine(directory, filename) |> Compact.deserializeFile |> Ok
                with e ->
                    Error
                    <| sprintf "Could not deserialise a file as %s (%s). %s" (typeof< ^T>.ToString()) filename e.Message
            else
                Error <| sprintf "File does not exist 1: %s" filename
        else
            Error <| sprintf "Directory does not exist 3: %s" directory

    let makeCacheFile<'a> directory filename (entity: 'a) =
        if Directory.Exists directory then
            Compact.serializeToFile (Path.Combine(directory, filename)) entity |> Ok
        else
            Error <| sprintf "Directory does not exist 2: %s" directory
    ...
    let makeCacheFileCompactList directory filename (entity: NodeIndexItem seq) =
        if Directory.Exists directory then
            ...
            with ex ->
                Error <| sprintf "%s - %s" ex.Message ex.StackTrace
        else
            Error <| sprintf "Directory does not exist 2: %s" directory

    /// Types that are indexed in a single file rather than individual files.
    let typesToIndex = [ typeof<Exposure.TemporalIndex.CalYearNode>.Name ]

    let loadIndex directory : Result<NodeIndexItem list, string> = loadCacheFile directory indexFile
    ...
    let loadAtom directory (atomType: string) (atomKey: Graph.UniqueKey) : Result<Graph.Atom<'a, 'b>, string> =
        if typesToIndex |> List.contains atomType then
            loadAtomsFromIndex directory atomType
            |> Result.bind (fun m ->
                m.TryFind atomKey
                |> Result.ofOption (sprintf "Could not find atom %s" atomKey.AsString))
        else
            loadCacheFile directory (sprintf "atom-%s.json" (atomKey.AsString.ToLower()))

    let loadAtoms
        directory
        (atomType: string)
        (atomKeys: Graph.UniqueKey list)
        : Result<list<Graph.Atom<'a, 'b>>, string> =
        if typesToIndex |> List.contains atomType then
            loadAtomsFromIndex directory atomType
    ...
    let saveAtomToIndex directory (atomType: string) atomKey item =
        ...
        if File.Exists <| Path.Combine(directory, file) then
            ...
            Map.empty |> Map.add atomKey item |> makeCacheFile directory file

    let saveAtom
        directory
        (atomType: string)
        (atomKey: Graph.UniqueKey)
        (item: (Graph.UniqueKey * GraphStructure.Node) * Graph.Adjacency<GraphStructure.Relation>)
        =
        if typesToIndex |> List.contains atomType then
            saveAtomToIndex directory atomType atomKey item
        else
            makeCacheFile directory (sprintf "atom-%s.json" (atomKey.AsString.ToLower())) item

    let saveAtomsToIndex
        directory
        (atomType: string)
        (items: seq<(Graph.UniqueKey * GraphStructure.Node) * Graph.Adjacency<GraphStructure.Relation>>)
        =
        let file = (sprintf "atom-%s-index.json" (atomType.ToLower()))

        let addItemsToMap m =
            Seq.fold
                (fun map (i: (Graph.UniqueKey * GraphStructure.Node) * Graph.Adjacency<GraphStructure.Relation>) ->
                    Map.add (i |> fst |> fst) i map)
                m
                items
        ...
        if File.Exists <| Path.Combine(directory, file) then
            ...
            Map.empty |> addItemsToMap |> Map.toArray |> makeCacheFile directory file
    ...
    let saveAtoms
        ...
        =
        if typesToIndex |> List.contains atomType then
            saveAtomsToIndex directory atomType items
        else
            items
            |> Seq.map (fun a ->
                makeCacheFile directory (sprintf "atom-%s.json" ((a |> fst |> fst).AsString.ToLower())) a)
            |> Seq.toList
            |> Result.ofList
            |> Result.lift (fun _ -> ())

    let initIndex directory =
        [] |> makeCacheFile directory indexFile |> Result.lift (fun _ -> [])
    ...
    let integrityCheckIndex directory =
        ...
        |> Result.bind (fun r ->
            r
            |> List.toArray
            |> Array.Parallel.map (fun indexItem ->
                if
                    indexItem.NodeTypeName = "SourceNode"
                    || indexItem.NodeTypeName = "TaxonNode"
                    || indexItem.NodeTypeName = "BioticProxyNode"
                then
                    loadAtom directory indexItem.NodeTypeName indexItem.NodeId
                    |> Result.lift
                        (fun
                            (atom: (Graph.UniqueKey * GraphStructure.Node) * Graph.Adjacency<GraphStructure.Relation>) ->
                            { indexItem with
    ...
    let nodesByType index =
        ...
        |> Seq.map (fun (g, l) -> g, (l |> Seq.map (fun n -> n.NodeId, n.PrettyName) |> Map.ofSeq))
        |> Map.ofSeq

    let loadOrInitGraph<'node, 'rel> directory =
        if not <| Directory.Exists directory then
            Error <| sprintf "Directory does not exist 1 (%s)" directory
        else
            let index =
                if File.Exists(Path.Combine(directory, indexFile)) then
                    loadIndex directory
                else
                    initIndex directory
    ...
    let atomFriendlyNameByKey<'c> key (graph: FileBasedGraph<GraphStructure.Node, GraphStructure.Relation>) =
        ...
        (unwrap graph).Graph
        |> Graph.getAtom key
        |> Option.map (fun n -> (n |> fst |> snd).DisplayName())

    /// Fetch a node by it's key
    let atomsByKey<'c> (graph: FileBasedGraph<GraphStructure.Node, GraphStructure.Relation>) keys =
        keys
        |> Seq.choose (fun guid -> (unwrap graph).Graph |> Seq.tryFind (fun (n, _) -> fst n = guid))
    ...
    let updateNode'
        updatedGraph
        (updatedAtom: (Graph.UniqueKey * GraphStructure.Node) * Graph.Adjacency<GraphStructure.Relation>)
        fileGraph
        =
        result {
            ...
            let! newIndex =
                if oldIndexedPrettyName <> (updatedAtom |> fst |> snd).DisplayName() then
                    result {
                        let! oldIndex = loadIndex (unwrap fileGraph).Directory

                        let! oldIndexItem =
                            oldIndex
                            |> Seq.tryFind (fun i -> i.NodeId = (updatedAtom |> fst |> fst))
                            |> Result.ofOption (
                                sprintf
                                    "Could not locate node id '%A' (%s) in node index"
                                    (updatedAtom |> fst |> fst)
                                    oldIndexedPrettyName
                            )
                        ...
                        return
                            Some(
                                oldIndex
                                |> List.except [ oldIndexItem ]
                                |> List.append [ newIndexItem ]
            ...
            if newIndex.IsSome then
                do! replaceIndex (unwrap fileGraph).Directory newIndex.Value
            else
                do! Ok()
            ...
            do!
                saveAtom
                    (unwrap fileGraph).Directory
                    ((updatedAtom |> fst |> snd).NodeType())
                    ((updatedAtom |> fst |> fst))
                    updatedAtom
            ...
            let updatedNodesByType =
                if newIndex.IsSome then
                    nodesByType newIndex.Value
                else
                    (unwrap fileGraph).NodesByType

            return
                FileBasedGraph
                    { (unwrap fileGraph) with
                        NodesByType = updatedNodesByType
                        Graph = updatedGraph }
        }

    /// Update the data associated with a node.
    let updateNode
        (fileGraph: FileBasedGraph<GraphStructure.Node, GraphStructure.Relation>)
        (node: Graph.UniqueKey * GraphStructure.Node)
        =
        result {
            ...
            let! updatedAtom =
                updatedGraph
                |> Graph.getAtom (fst node)
                |> Result.ofOption (sprintf "Could not find atom %O" (fst node))
    ...
    let addRelation
        ...
        result {
            let! updatedGraph =
                GraphStructure.Relations.addRelation sourceAtom sinkAtom relation 1 (unwrap fileGraph).Graph

            let! updatedSourceAtom =
                updatedGraph
                |> Graph.getAtom (sourceAtom |> fst |> fst)
                |> Result.ofOption (sprintf "Could not find atom: %O" (sourceAtom |> fst |> fst))

            return! updateNode' updatedGraph updatedSourceAtom fileGraph
        }

    /// Add a relationship between two nodes.
    let addRelationByKey
        (fileGraph: FileBasedGraph<GraphStructure.Node, GraphStructure.Relation>)
        (source: Graph.UniqueKey)
        (sink: Graph.UniqueKey)
        relation
        =
        result {
    ...
    let addNodes' addFn (fileGraph: FileBasedGraph<GraphStructure.Node, GraphStructure.Relation>) nodes =
        ...
        |> Result.bind (fun (updatedGraph, addedNodes) ->
            ...
            if newAtoms.Length <> (nodes |> Seq.length) then
                Error "Problem saving new graph nodes."
            else
                let saveNodes () =
                    newAtoms
                    |> List.groupBy (fun ((_, (n: GraphStructure.Node)), _) -> n.NodeType())
                    |> List.map (fun (nodeType, atoms) -> saveAtoms (unwrap fileGraph).Directory nodeType atoms)
                    |> Result.ofList
    ...
    let seedGraph (fileGraph: FileBasedGraph<GraphStructure.Node, GraphStructure.Relation>) =
        result {
            ...
            let groupedByType = seeded |> Seq.groupBy (fun (node, _) -> (snd node).NodeType())
    ...
    let tryFindTaxonByName taxon graph =
        let latinName =
            ...
            | Population.Taxonomy.Subspecies(g, s, ssp, a) ->
                sprintf "%s %s ssp. %s" g.Value s.Value ssp.Value
                |> FieldDataTypes.Text.createShort
            | Population.Taxonomy.Variety(g, s, ssp, a) ->
                sprintf "%s %s var. %s" g.Value s.Value ssp.Value
                |> FieldDataTypes.Text.createShort

        (unwrap graph).Graph
        |> GraphStructure.Nodes.tryFindTaxon (fun t ->
    ...
    let addProxiedTaxon' (edge: Population.ProxiedTaxon.ProxiedTaxonHyperEdge) graph =
        result {
            // Get the three included nodes:
            let! existingProxy =
                (unwrap graph).Graph
                |> GraphStructure.Nodes.tryFindProxy (fun n -> n = edge.InferredFrom),
                "proxy doesn't exist"

            let! existingInfer =
                (unwrap graph).Graph
                |> GraphStructure.Nodes.tryFindInferenceMethod (fun n -> n = edge.InferredUsing),
                "infer doesn't exist"
            ...
            let! proxiedGraph, addedNodes =
                addNodes graph [ GraphStructure.PopulationNode GraphStructure.ProxiedTaxonNode ]
            ...
            return!
                ...
                |> addRelation
                    addedNodes.Head
                    existingProxy
                    (GraphStructure.ProposedRelation.Population Population.PopulationRelation.InferredFrom)
                |> Result.bind (
                    addRelation
                        addedNodes.Head
                        existingProxy
                ...
                |> Result.bind (
                    addRelation
                        addedNodes.Head
                        existingInfer
                        (GraphStructure.ProposedRelation.Population Population.PopulationRelation.InferredUsing)
                )
    ...
    let addProxiedTaxon existingProxy existingTaxon existingAdditionalTaxa existingInfer fileGraph =
        if
            (existingTaxon :: existingAdditionalTaxa) |> List.distinct |> List.length
            <> ((existingTaxon :: existingAdditionalTaxa) |> List.length)
        then
            Error "Cannot add proxied taxon inferred as the same taxon more than once"
        else
            addProxiedTaxon'
                { InferredFrom = existingProxy
                  InferredUsing = existingInfer
                  InferredAs = existingTaxon
                  InferredAsAdditional = existingAdditionalTaxa }
                fileGraph
