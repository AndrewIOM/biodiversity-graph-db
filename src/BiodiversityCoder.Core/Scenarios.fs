...
module NodeSelection =
    ...
    let trySelectTime
        ...
        if nearestYear <= 11650 then
            trySelectTimeNode graph value |> Option.map (fun (a, b) -> (a, b, false))
        else
            Some(
                Graph.FriendlyKey(
...
module Scenarios =
    ...
    and WoodRingScenario =
        ...
        static member Description =
            "For sources that construct standard tree or shrub ring chronologies, use this scenario to quickly enter details for a particular site."

    and WoodTaxon =
        | Genus of genus: Text.ShortText
        | Species of generic: Text.ShortText * specific: Text.ShortText * authorship: Text.ShortText
        | Subspecies of
            generic: Text.ShortText *
            specific: Text.ShortText *
            subspecific: Text.ShortText *
            authorship: Text.ShortText
    ...
    and SiteOnlyScenario =
        ...
        static member Description =
            "Leaves the following details for adding later: individual taxon records, individual dates."
            ...
            result {
                ...
                let! startDateNode, startDateRel, (startDateActual: OldDate.OldDateSimple) =
                    ...
                    |> Option.bind (fun (k: Graph.UniqueKey, v, outOfScope) ->
                        ...
                        |> Option.map (fun n ->
                            if outOfScope then
                                n, Exposure.ExposureRelation.ExtentEarliestOutOfScope v, v
                            else
                                n, Exposure.ExposureRelation.ExtentEarliest, v))
                ...
                let addDateUncertainties timelineNode g =
                    ...
                    | OldDate.MeasurementError.DatingErrorPlusMinus error ->
                        let earlyEarly =
                            NodeSelection.holoceneCalYear startDateActual + removeUnit error
                            |> NodeSelection.trySelectTimeNodeByYear g

                        let earlyLate =
                            NodeSelection.holoceneCalYear startDateActual - removeUnit error
                            |> NodeSelection.trySelectTimeNodeByYear g

                        if earlyEarly.IsNone || earlyLate.IsNone then
                            Error "Could not assign the uncertainty value for the earliest year."
                        else
                            Storage.addRelationByKey
                                g
                                (timelineNode |> fst |> fst)
                                earlyEarly.Value
                                (ProposedRelation.Exposure Exposure.ExposureRelation.ExtentEarliestUncertainty)
                            |> Result.bind (fun g ->
                                Storage.addRelationByKey
                                    g
                                    (timelineNode |> fst |> fst)
                ...
                let addDateUncertaintiesLate timelineNode g =
                    ...
                    | OldDate.MeasurementError.DatingErrorPlusMinus error ->
                        let lateEarly =
                            NodeSelection.holoceneCalYear endDateActual + removeUnit error
                            |> NodeSelection.trySelectTimeNodeByYear g
                        ...
                        if lateEarly.IsNone || lateLate.IsNone then
                            Error "Could not assign the uncertainty value for the earliest year."
                        else
                            Storage.addRelationByKey
                                g
                                (timelineNode |> fst |> fst)
                                lateEarly.Value
                                (ProposedRelation.Exposure Exposure.ExposureRelation.ExtentLatestUncertainty)
                            |> Result.bind (fun g ->
                                Storage.addRelationByKey
                                    g
                                    (timelineNode |> fst |> fst)
                ...
                let! newGraph =
                    ...
                    |> Result.bind (fun (g, addedNodes) ->
                        let timelineNode =
                            addedNodes
                            |> Seq.find (fun s -> (s |> fst |> snd).NodeType() = "IndividualTimelineNode")

                        let contextNode =
                            addedNodes |> Seq.find (fun s -> (s |> fst |> snd).NodeType() = "ContextNode")

                        Storage.addRelation
                            sourceNode
                            timelineNode
                            (ProposedRelation.Source Sources.SourceRelation.HasTemporalExtent)
                            g
                        |> Result.bind (fun g ->
                            Storage.addRelation timelineNode startDateNode (ProposedRelation.Exposure startDateRel) g)
                        |> Result.bind (fun g ->
                            Storage.addRelation
                                timelineNode
                                endDateNode
                                (ProposedRelation.Exposure Exposure.ExposureRelation.ExtentLatest)
                                g)
                        ...
                        |> Result.bind (fun g ->
                            Storage.addRelation
                                timelineNode
                                contextNode
                                (ProposedRelation.Exposure Exposure.ExposureRelation.IsLocatedAt)
                                g)
                                ...
                                (fun state proxy ->
                                    ...
                                    |> Result.bind (fun gr ->
                                        Storage.addRelationByKey
                                            ...
                                            (timelineNode |> fst |> fst)
                                            (makeUniqueKey proxy)
                                            (ProposedRelation.Exposure Exposure.ExposureRelation.HasProxyCategory)))
                                (Ok g)
                                proxyNodes))
            ...
            result {
                ...
                let! startDateNode =
                    Storage.atomByKey
                        (Graph.UniqueKey.FriendlyKey(
                            "calyearnode",
                            sprintf "%iybp" <| 1950 - (removeUnit vm.EarliestYear)
                        ))
                        graph
                    |> Result.ofOption ""
                ...
                let! endDateNode =
                    Storage.atomByKey
                        (Graph.UniqueKey.FriendlyKey(
                            "calyearnode",
                            sprintf "%iybp" <| 1950 - (removeUnit vm.LatestYear)
                        ))
                        graph
                    |> Result.ofOption ""

                let! collectionDateNode =
                    Storage.atomByKey
                        (Graph.UniqueKey.FriendlyKey(
                            "calyearnode",
                            sprintf "%iybp" <| 1950 - (removeUnit vm.CollectionDate)
                        ))
                        graph
                    |> Result.ofOption ""

                let! measureNode =
                    Storage.atomByKey (Graph.UniqueKey.FriendlyKey("biodiversitydimensionnode", "presence")) graph
                    |> Result.ofOption ""

                let! proxyTaxon =
                    match vm.Taxon with
                    | Genus g ->
                        sprintf "%s sp." g.Value
                        |> Text.createShort
                        |> Result.lift BioticProxies.ContemporaneousWholeOrganism
                    | Species(g, s, auth) ->
                        (sprintf "%s %s %s" g.Value s.Value auth.Value)
                        |> Text.createShort
                        |> Result.lift BioticProxies.ContemporaneousWholeOrganism
                    | Subspecies(g, s, ssp, auth) ->
                ...
                let! existingTaxonNode =
                    ...
                    | Species(g, s, auth) ->
                        makeUniqueKey (PopulationNode(TaxonomyNode(Taxonomy.TaxonNode.Species(g, s, auth))))
                    | Subspecies(g, s, ssp, auth) ->
                        makeUniqueKey (PopulationNode(TaxonomyNode(Taxonomy.TaxonNode.Subspecies(g, s, ssp, auth))))
                ...
                let! newGraph =
                    graph
                    |> fun g -> Storage.addNodes g [ timelineNode; individualDate; contextNode ]
                    |> Result.bind (fun (g, addedNodes) ->
                        let timelineNode =
                            addedNodes
                            |> Seq.find (fun s -> (s |> fst |> snd).NodeType() = "IndividualTimelineNode")
                        ...
                        let dateNode =
                            addedNodes
                            |> Seq.find (fun s -> (s |> fst |> snd).NodeType() = "IndividualDateNode")
                        ...
                        |> Result.bind (fun g ->
                            Storage.addRelation
                                timelineNode
                                startDateNode
                                (ProposedRelation.Exposure Exposure.ExposureRelation.ExtentEarliest)
                                g)
                        |> Result.bind (fun g ->
                            Storage.addRelation
                                timelineNode
                        ...
                        |> Result.bind (fun g ->
                            Storage.addRelation
                                dateNode
                                collectionDateNode
                                (ProposedRelation.Exposure
                                 <| Exposure.ExposureRelation.TimeEstimate(
                                     OldDate.OldDateSimple.HistoryYearAD((float vm.CollectionDate) * 1.<OldDate.AD>)
                                 ))
                                g)
                        |> Result.bind (fun g ->
                            Storage.addRelation
                                timelineNode
                                dateNode
                                (ProposedRelation.Exposure Exposure.ExposureRelation.ConstructedWithDate)
                                g)
                        |> Result.bind (fun g ->
                            Storage.addRelation
                                timelineNode
                        ...
                        |> Result.bind (fun (g, proxiedKey) ->
                            Storage.addRelationByKey
                                ...
                                (timelineNode |> fst |> fst)
                                proxiedKey
                                (ProposedRelation.Exposure Exposure.ExposureRelation.HasProxyInfo)
                        ...
                        |> Result.bind (fun (g, proxiedKey) ->
                            Storage.addRelationByKey
                                ...
                                proxiedKey
                                (measureNode |> fst |> fst)
                                (ProposedRelation.Population <| Population.PopulationRelation.MeasuredBy)))
