...
type FormMessage =
    ...
    | AddOrUpdateNode of
        System.Type *
        validateRelations: (seq<GraphStructure.ProposedRelation> -> bool) *
        requiredRelations: RelationViewModel list
    | EnterNodeRelationData of
        nodeType: string *
        toggle: string *
        GraphStructure.ProposedRelation *
        name: string *
        sinkKeys: Graph.UniqueKey list
    | ChangeNodeRelationToggle of nodeType: string * toggle: string
    /// Allows entry of a value from which to compute the relationship
    | EnterRelationCreationData of
        nodeType: string *
        toggle: string *
        fieldName: string *
        enteredValue: NodeViewModel *
        rel: GraphStructure.ProposedRelation option
...
module Create =
    ...
    let processField field propertyType createFromViewModel =
        ...
        | DU(case, vm2) ->
            ...
            if FSharpType.IsUnion(propertyType) then
                ...
                Error "The type is not a DU as specified in the view model"
    ...
    let fieldName (field: System.Reflection.PropertyInfo) =
        ...
        | true -> field.Name
        | false ->
            (field.GetCustomAttributes(typeof<NameAttribute>, true) |> Seq.head :?> NameAttribute)
                .value
    ...
    let rec createFromViewModel (objType: System.Type) (viewModel: NodeViewModel) : Result<obj, string> =
        ...
        | DU(case1, newValue) ->
            ...
            | Some caseInfo ->
                ...
                let args: Result<obj list, string> =
                    (match newValue with
                     ...
                     | DU(case2, newValue2) ->
                         ...
                         | 1 ->
                             ...
                             // | None -> Error "The DU case as specified in the view model does not exist"
                                 ...
                                 Error "A non-DU type was specified on a DU field, but only a DU view model was given"
                         | _ ->
                             Error
                                 "A DU view model was specified for a DU, but the parent DU does not have a single field."
                     | FieldValue _ ->
                     ...
                     | FieldList fl ->
                         match caseInfo.GetFields().Length with
                         | 1 ->
                             if (objType.IsGenericType && objType.GetGenericTypeDefinition() = typedefof<_ list>) then
                ...
                args
                |> Result.lift (fun args -> Reflection.FSharpValue.MakeUnion(caseInfo, args |> List.toArray))
        ...
        | FieldValue v ->
            ...
            | 1 ->
                ...
                let created =
                    objType.InvokeMember("TryCreate", BindingFlags.InvokeMethod, null, null, [| v |])
        ...
        | Fields newFields ->
            ...
            if Reflection.FSharpType.IsRecord(objType) then
                ...
                let values =
                    recordFields
                    |> Array.map (fun f ->
                        if newFields |> Map.containsKey f.Name then
                            processField (newFields.[f.Name]) f.PropertyType createFromViewModel
                        else
                            Error(sprintf "No data has been entered for: %s" (fieldName f)))
                    |> Array.toList
                    |> Result.ofList
                ...
                Error "Not a list or a record type"
        | FieldList fl ->
            if (objType.IsGenericType && objType.GetGenericTypeDefinition() = typedefof<_ list>) then
                // Is a list<'a>. Collect 'fields' into items for the list.
                let listInstanceType = objType.GetProperty("Head").PropertyType
...
module Merge =
    ...
    let rec updateNodeViewModel master (newMaster: NodeViewModel) =
        ...
        | DU(case1, newValue) ->
            ...
            | DU(case2, oldValue) ->
                if case1 = case2 then
                    DU(case2, updateNodeViewModel oldValue newValue)
                else
                    DU(case1, updateNodeViewModel NotEnteredYet newValue)
        ...
        | FieldList fl ->
            ...
            | FieldList oldFl ->
                let newData =
                    ...
                    | Some oldField ->
                        if snd fl.Head = FieldValue(Text "--REMOVE LIST ITEM--") then
                            oldFl |> List.except [ oldField ]
                        else
                            oldFl
                            |> List.except [ oldField ]
                            |> List.append [ (fst oldField, updateNodeViewModel (snd oldField) (snd fl.Head)) ]
                    | None ->
                        if snd fl.Head = FieldValue(Text "--REMOVE LIST ITEM--") then
                            oldFl
                        else
                            oldFl |> List.append [ fl.Head ]
                    |> List.sortBy fst
...
module ViewGen =
    ...
    let genUnits (field: System.Reflection.PropertyInfo) =
        cond (field.GetCustomAttributes(typeof<UnitAttribute>, true) |> Seq.isEmpty)
        <| function
            | true -> empty ()
            | false ->
                span {
                    ...
                    text
                        (field.GetCustomAttributes(typeof<UnitAttribute>, true) |> Seq.head :?> HelpAttribute)
                            .Text
                }

    /// Generate help text from the `Help` attribute if specified.
    let genHelp (field: System.Reflection.PropertyInfo) =
        cond (field.GetCustomAttributes(typeof<HelpAttribute>, true) |> Seq.isEmpty)
        <| function
            | true -> empty ()
            | false ->
                small {
                    ...
                    text
                        (field.GetCustomAttributes(typeof<HelpAttribute>, true) |> Seq.head :?> HelpAttribute)
                            .Text
                }

    /// Generate the name from the `Name` attribute if specified.
    let genName (field: System.Reflection.PropertyInfo) =
        cond (field.GetCustomAttributes(typeof<NameAttribute>, true) |> Seq.isEmpty)
        <| function
        ...
        div {
            ...
            div {
                ...
                select {
                    ...
                    forEach (Reflection.FSharpType.GetUnionCases(t))
                    <| fun c ->
                        option {
                            attr.value c.Name
                            text c.Name
        ...
        div {
            ...
            div {
                ...
                let f =
                    ...
                    | t when t = typeof<FieldDataTypes.Text.ShortText> ->
                        genStringInput field existingValue 100 dispatch
                    | t when t = typeof<FieldDataTypes.Text.Text> -> genStringInput field existingValue 9999 dispatch
                    | t when t = typeof<FieldDataTypes.LanguageCode.LanguageCode> ->
                        genStringInput field existingValue 2 dispatch
                    | t when t = typeof<FieldDataTypes.Geography.Polygon> ->
                        genStringInput field existingValue 1000 dispatch
                    | t when t = typeof<FieldDataTypes.Geography.CoordinateDMS> ->
                        genStringInput field existingValue 100 dispatch
                    | t when t = typeof<FieldDataTypes.Author.Author> ->
                        genStringInput field existingValue 2000 dispatch
                    | t when t = typeof<FieldDataTypes.DigitalObjectIdentifier.DigitalObjectIdentifier> ->
                        genStringInput field existingValue 150 dispatch
                    | t when t = typeof<FieldDataTypes.IntRange.IntRange> ->
                        genStringInput field existingValue 50 dispatch
                    | t when t = typeof<FieldDataTypes.StratigraphicSequence.Depth> ->
                        genFloatInput field existingValue dispatch
    ...
    let rec renderPropertyInfo
        f
        (field: System.Reflection.PropertyInfo)
        (nestedVm: NodeViewModel -> NodeViewModel)
        hideLabel
        (dispatch: NodeViewModel -> unit)
        makeField'
        =
        // Figure out if the field already has a value.
        cond (f |> Map.tryFind field.Name)
        <| function
            | Some v ->
                ...
                <| function
                        ...
                        listGroup
                            ...
                            [ forEach l
                              <| fun k ->
                                  concat {
                                      renderPropertyInfo
                                          (Map.ofList [ "Head", snd k ])
                                          (field.PropertyType.GetProperty("Head"))
                                          (fun vm -> nestedVm <| FieldList([ fst k, vm ]))
                                          true
                                          dispatch
                                          makeField'
                                      ...
                                      small {
                                          on.click (fun _ ->
                                              (nestedVm (FieldList [ fst k, FieldValue(Text "--REMOVE LIST ITEM--") ]))
                                              |> dispatch)
                                      ...
                                      button {
                                          ...
                                          on.click (fun _ ->
                                              (nestedVm (
                                                  FieldList [ (l |> List.map fst |> List.max) + 1, NotEnteredYet ]
                                              ))
                                              |> dispatch)
                    ...
                    | Fields _
                    | NotEnteredYet ->
                        cond (isList field.PropertyType)
                        <| function
                            | true ->
                                listGroup
                                    field
                            ...
                            | false ->
                                ...
                                cond (Reflection.FSharpType.IsUnion(field.PropertyType))
                                <| function
                                    | true ->
                                    ...
                                    | false ->
                                        // A field is not a DU.
                                        cond (Reflection.FSharpType.IsRecord(field.PropertyType))
                                        <| function
                                            | true ->
                                                // Is a record. Render fields nested.
                                                forEach (Reflection.FSharpType.GetRecordFields(field.PropertyType))
                                                <| fun field ->
                                                    renderPropertyInfo
                                                        f
                                                        field
                                                        (fun vm -> nestedVm <| Fields([ field.Name, vm ] |> Map.ofList))
                                                        false
                                                        dispatch
                                                        makeField'
                                            | false ->
    ...
    let rec makeField
        (formId: string)
        (field: System.Reflection.PropertyInfo option)
        viewModel
        (nestedVm: NodeViewModel -> NodeViewModel)
        nestedType
        dispatch
        =
        cond (Reflection.FSharpType.IsUnion(nestedType))
        <| function
            | true ->
                ...
                concat {
                    ...
                    <| function
                        | DU(selectedCase: string, vm) ->
                            ...
                            concat {
                                genSelect field nestedType selectedCase (fun (s: string) ->
                                    (nestedVm (DU(s, vm))) |> dispatch)

                                cond (
                                    Reflection.FSharpType.GetUnionCases(nestedType)
                                    |> Seq.tryFind (fun c -> c.Name = selectedCase)
                                )
                                <| function
                                    | Some s ->
                                                        ...
                                                        | None ->
                                                            renderPropertyInfo
                                                                ...
                                                                (fun vm ->
                                                                    ...
                                                                    <| DU(
                                                                        selectedCase,
                                                                        Fields([ field.Name, vm ] |> Map.ofList)
                                                                    ))
                                                                false
                                                                dispatch
                                                                (makeField formId)
                        ...
                        | _ ->
                            if Reflection.FSharpType.GetUnionCases(nestedType).Length <> 1 then
                                ...
                                cond (
                                    Reflection.FSharpType.GetUnionCases(nestedType)
                                    |> Seq.tryFind (fun c ->
                                        c.Name = (Reflection.FSharpType.GetUnionCases(nestedType).[0].Name))
                                )
                                <| function
                                    | Some s ->
                                        // None of the fields have any entered values yet. Render all of them cleanly.
                                        ...
                                        <| fun field ->
                                            renderPropertyInfo
                                                ...
                                                (fun vm ->
                                                    nestedVm
                                                    <| DU(
                                                        (Reflection.FSharpType.GetUnionCases(nestedType).[0].Name),
                                                        Fields([ field.Name, vm ] |> Map.ofList)
                                                    ))
                                                false
                                                dispatch
            ...
            | false -> // Is not a DU.
                cond (Reflection.FSharpType.IsRecord(nestedType))
                <| function
                    | true -> // Is an F# record. Generate display for all fields.
                        ...
                        <| function
                            | Fields f ->
                                ...
                                forEach (Reflection.FSharpType.GetRecordFields(nestedType))
                                <| fun field ->
                                    renderPropertyInfo
                                        f
                                        field
    ...
    let makeNodeForm'<'a>
        (nodeViewModel: NodeViewModel option)
        buttonName
        dispatch
        validateRelations
        (requiredRelations: RelationViewModel list)
        ...
        div {
            ...
            <| function
                | Some vm ->
                    makeField (typeof<'a>).Name None vm id typeof<'a> (fun vm ->
                        EnterNodeCreationData((typeof<'a>).Name, vm) |> dispatch)
                | None ->
                    makeField (typeof<'a>).Name None NotEnteredYet id typeof<'a> (fun vm ->
                        EnterNodeCreationData((typeof<'a>).Name, vm) |> dispatch)
    ...
    let makeNodeFormWithRelations<'a>
        validateRelations
        (nodeViewModel: NodeViewModel option)
        requiredRelations
        dispatch
    ...
    module RelationsForms =
        ...
        let selectExistingNode<'sinkNodeType>
            name
            helpText
            (rel: GraphStructure.ProposedRelation)
            sourceNodeName
            toggle
            (relationValues: Map<string * GraphStructure.ProposedRelation, Graph.UniqueKey list>)
            graph
            dispatch
            =
            div {
                ...
                div {
                    ...
                    cond (relationValues |> Map.tryFind (name, rel))
                    <| function
                        | Some v ->
                            cond v.IsEmpty
                            <| function
                                ...
                                | false ->
                                    select {
                                        ...
                                        bind.change.string v.Head.AsString (fun v ->
                                            EnterNodeRelationData(
                                                sourceNodeName,
                                                toggle,
                                                rel,
                        ...
                        | None ->
                            select {
                                ...
                                bind.change.string "" (fun v ->
                                    EnterNodeRelationData(sourceNodeName, toggle, rel, name, [ Graph.stringToKey v ])
                                    |> dispatch)
        ...
        let selectExistingBy<'sinkNodeType, 'relArg>
            name
            helpText
            (rel: 'relArg -> GraphStructure.ProposedRelation)
            (tryCompute: 'relArg -> (Graph.UniqueKey * 'relArg) option)
            (relationVms: Map<string * string, Map<string, NodeViewModel * GraphStructure.ProposedRelation option>>)
            sourceNodeName
            toggle
            (relationValues: Map<string * GraphStructure.ProposedRelation, Graph.UniqueKey list>)
            graph
            ...
            div {
                ...
                div {
                    ...
                    cond (relationVms |> Map.tryFind (sourceNodeName, toggle))
                    <| function
                        | Some existing ->
                            cond (existing |> Map.tryFind name)
                            <| function
                                | Some(vm, proposed) ->
                                    cond proposed
                                    <| function
                                        | Some p ->
                                            ...
                                            cond (relationValues |> Map.tryFind (name, p))
                                            <| function
                                                | Some all ->
                                                    concat {
                                                        forEach all <| fun v -> textf "Selected node is %s" v.AsString
                                        ...
                                        | None ->
                                            ...
                                            concat {
                                                makeField (typeof<'relArg>).Name None vm id typeof<'relArg> (fun vm ->
                                                    EnterRelationCreationData(sourceNodeName, toggle, name, vm, None)
                                                    |> dispatch)
                                                ...
                                                button {
                                                    ...
                                                    on.click (fun _ ->
                                                        ...
                                                        |> Result.bind (
                                                            tryCompute >> Result.ofOption "Could not create a link"
                                                        )
                                ...
                                | None ->
                                    makeField (typeof<'relArg>).Name None NotEnteredYet id typeof<'relArg> (fun vm ->
                                        EnterRelationCreationData(sourceNodeName, toggle, name, vm, None) |> dispatch)
                        | None ->
                            makeField (typeof<'relArg>).Name None NotEnteredYet id typeof<'relArg> (fun vm ->
                                EnterRelationCreationData(sourceNodeName, toggle, name, vm, None) |> dispatch)
        ...
        let selectExistingNodeMulti<'sinkNodeType>
            (name: string)
            helpText
            (rel: GraphStructure.ProposedRelation)
            sourceNodeName
                                    ...
                                    select {
                                        ...
                                        bind.change.string v.AsString (fun v ->
                                            EnterNodeRelationData(
                                                ...
                                                rel,
                                                name,
                                                (Graph.stringToKey v :: (all |> List.except [ Graph.stringToKey v ]))
                                            )
                                            |> dispatch)
        ...
        let relationsToggle<'a>
            name
            (elements:
                (string *
                (string
                    -> string
                    -> Map<string * GraphStructure.ProposedRelation, Graph.UniqueKey list>
                    -> Storage.FileBasedGraph<GraphStructure.Node, GraphStructure.Relation>
                    -> (FormMessage -> unit)
                    -> Bolero.Node) list) list)
            (currentRelations:
                Map<string, string * Map<string * GraphStructure.ProposedRelation, list<Graph.UniqueKey>>>)
            graph
            dispatch
            =
            cond (currentRelations |> Map.tryFind (typeof<'a>.Name))
            <| function
                | Some(toggleSet, relationValues) ->
                    concat {
                        cond (elements.Length > 1)
                        <| function
                            | true ->
                                div {
                                    _class "row mb-3"
                                                ...
                                                <| function
                                                    | true ->
                                                        button {
                                                            ...
                                                            on.click (fun _ ->
                                                                ChangeNodeRelationToggle(typeof<'a>.Name, toggle)
                                                                |> dispatch)
                        ...
                        cond (elements |> Seq.tryFind (fun (e, _) -> e = toggleSet))
                        <| function
                            | Some(_, n) ->
                                concat {
                                    forEach n
                                    <| fun n -> n (typeof<'a>.Name) toggleSet relationValues graph dispatch
                                }
                            | None -> empty ()
                    }
                | None ->
                    concat {
                        ...
                        forEach (snd elements.Head)
                        <| fun n -> n (typeof<'a>.Name) (fst elements.Head) Map.empty graph dispatch
                    }
        ...
        module Validation =
            ...
            let hasOneByCase (case: string) (relations: GraphStructure.ProposedRelation seq) =
                ...
                |> Seq.where (fun r ->
                    let info2, values =
                        FSharpValue.GetUnionFields(r, typeof<GraphStructure.ProposedRelation>)

                    let info3 = info2.GetFields()

                    if info3.Length <> 1 then
                        false
                    else
            ...
            let hasOne i relations =
                relations |> Seq.where (fun r -> r = i) |> Seq.length = 1
