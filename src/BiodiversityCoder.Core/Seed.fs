...
module Seed =
    ...
    let testHyperEdge (graph: Graph.Graph<Node, Relation>) =
        result {
            ...
            let proxyNode =
                BioticProxies.Morphotype(BioticProxies.Microfossil(BioticProxies.Pollen, morphotype))

            let! inferSource = Text.create "Moore 1987. Pollen of the British Isles."

            let inferNode =
                BioticProxies.InferenceMethodNode.IdentificationKeyOrAtlas inferSource
            ...
            return!
                ...
                |> Graph.addNodeData
                    ...
                    [ PopulationNode(BioticProxyNode proxyNode)
                      ...
                      PopulationNode(TaxonomyNode taxonNode) ]
                |> Result.lift fst
                |> Result.bind (
    ...
    let initGraph () =
        ...
        let holoceneLabel =
            SliceLabelNode
                { Name = forceOk (Text.createShort "Holocene")
                  DesignatingAuthority = forceOk (Text.createShort "Global Stratotype Section and Point") }
        ...
        let graph: Result<Graph.Graph<Node, Relation>, string> =
            Graph.empty
            |> Graph.addNodeData makeUniqueKey [ PopulationNode(TaxonomyNode taxonRoot); ExposureNode holoceneLabel ]
            |> Result.lift fst
        ...
        // Add core relations
        let addTimeRelation year relation graph =
            result {
                let! source =
                    Nodes.tryFindTimePeriod (forceOk (Text.createShort "Holocene")) graph, "Could not get Holocene node"
