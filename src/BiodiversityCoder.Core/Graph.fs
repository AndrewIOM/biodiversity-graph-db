...
module Graph =
    ...
    type Adjacency<'TStructon> = Connection<'TStructon> list

    /// A single node with its outgoing connections
    type Atom<'TNodeDaton, 'TConnectionDaton> = Node<'TNodeDaton> * Adjacency<'TConnectionDaton>

    type Graph<'TNodeDaton, 'TConnectionDaton> = Atom<'TNodeDaton, 'TConnectionDaton> list

    let empty: Graph<_, _> = []

    let getAtomId atom = atom |> fst |> fst

    let getAtom id graph =
        graph |> List.tryFind (fun atom -> atom |> getAtomId = id)
    ...
    let pointsTo id atom =
        atom |> snd |> List.exists (fun (_, y, _, _) -> y = id)

    let getConnectionSinkId ((_, sinkId, _, _): Connection<_>) = sinkId
    ...
    let addRelation
        ...
        connData
        (graph: Graph<_, _>)
        : Result<Graph<'nodeData, 'connData>, string> =
        let source = graph |> List.tryFind (fun n -> fst (fst n) = sourceId)
        let sink = graph |> List.tryFind (fun n -> fst (fst n) = sourceId)

        if source.IsNone || sink.IsNone then
            Error "The connection was not between valid nodes"
        else
            graph
            |> List.map (fun i ->
                if i = source.Value then
                    let conn: Connection<'connData> = (sourceId, sinkId, weight, connData)

                    let adjacency =
                        if (snd i) |> Seq.contains conn then
                            snd source.Value
                        else
                            conn :: snd source.Value
    ...
    let tryFind cond (graph: Graph<_, _>) = graph |> Seq.tryFind (fun g -> cond g)
...
module GraphStructure =
    ...
    type OutcomeNode = MeasureNode of Biodiversity.BiodiversityDimensionNode
    ...
    let tryAlphanum (c: char) =
        if System.Char.IsLetter c || System.Char.IsNumber c then
            Some c
        else
            None
        ...
        member this.DisplayName() =
            ...
            | PopulationNode p ->
                ...
                | BioticProxyNode n ->
                    ...
                    | Population.BioticProxies.BioticProxyNode.ContemporaneousWholeOrganism taxon ->
                        sprintf "Contemporaneous Whole Organism: %s" taxon.Value
                    | Population.BioticProxies.BioticProxyNode.Morphotype m ->
                        match m with
                        | Population.BioticProxies.Megafossil(part, f) ->
                            sprintf "Megafossil: %s (%s)" f.Value part.Value
                        | Population.BioticProxies.Macrofossil(part, f) ->
                            sprintf "Macrofossil: %s (%s)" f.Value part.Value
                        | Population.BioticProxies.Microfossil(group, name) ->
                            match group with
                            | Population.BioticProxies.MicrofossilGroup.Diatom ->
                                sprintf "Morphotype: Diatom - %s" name.Value
                            | Population.BioticProxies.MicrofossilGroup.Ostracod ->
                                sprintf "Morphotype: Ostracod - %s" name.Value
                            | Population.BioticProxies.MicrofossilGroup.PlantMacrofossil ->
                                sprintf "Morphotype: Plant Macrofossil - %s" name.Value
                            | Population.BioticProxies.MicrofossilGroup.Pollen ->
                                sprintf "Morphotype: Pollen - %s" name.Value
                            | Population.BioticProxies.MicrofossilGroup.OtherMicrofossilGroup group ->
                                sprintf "Morphotype: %s - %s" group.Value name.Value
                ...
                | TaxonomyNode n ->
                    ...
                    | Taxonomy.TaxonNode.Subspecies(l, l2, l3, l4) ->
                        sprintf "%s %s subsp. %s %s [Subspecies]" l.Value l2.Value l3.Value l4.Value
                    | Taxonomy.TaxonNode.Variety(l, l2, l3, l4) ->
                        sprintf "%s %s var. %s %s [Variety]" l.Value l2.Value l3.Value l4.Value
                | InferenceMethodNode n ->
                    match n with
                    | BioticProxies.InferenceMethodNode.Implicit -> "Implicit"
                    | BioticProxies.InferenceMethodNode.IdentificationKeyOrAtlas r ->
                        sprintf "Explicit: Atlas or Key - %s" r.Value
                    | BioticProxies.InferenceMethodNode.ImplicitByExpert(lastName, initials) ->
                        sprintf "Implicit: Expert ID - %s, %s" lastName.Value initials.Value
            ...
            | SourceNode s ->
                ...
                | Excluded(s, _, _) ->
                    ...
                    | DatabaseEntry n ->
                        sprintf
                            "Database: %s - entry %s"
                            n.DatabaseAbbreviation.Value
                            n.UniqueIdentifierInDatabase.Value
                    | PublishedSource p ->
                        match p with
                        | Book b ->
                            sprintf
                                "%s (%i). [book] %s. %s"
                                (FieldDataTypes.Author.authorList (
                                    List.concat [ [ b.BookFirstAuthor ]; b.BookAdditionalAuthors ]
                                ))
                                b.BookCopyrightYear
                                b.BookTitle.Value
                                (if b.ISBN.IsSome then
                                     b.ISBN.Value.Value + " (ISBN)"
                                 else if b.ISSNDOI.IsSome then
                                     b.ISSNDOI.Value.Value + " (ISSN/DOI)"
                                 else
                                     "")
                        | BookChapter n ->
                            sprintf
                                "%s. [book chapter] %s"
                                (FieldDataTypes.Author.authorList (
                                    List.concat [ [ n.ChapterFirstAuthor ]; n.ChapterAdditionalAuthors ]
                                ))
                        ...
                        | IndividualDataset d ->
                            sprintf
                                "%s (%s). [dataset] %s."
                                (FieldDataTypes.Author.authorList d.Contributors)
                                (if d.YearPublished.IsSome then
                                     d.YearPublished.Value.ToString()
                                 else
                                     "Unknown year")
                                d.Title.Value
                        | JournalArticle n ->
                    ...
                    | GreyLiteratureSource n ->
                        sprintf
                            ...
                            (if n.PostedYear.IsSome then
                                 n.PostedYear.Value.ToString()
                             else
                                 "Unknown year")
                            (n.Format.ToString())
                            n.Title.Value
                            (if n.Institution.IsSome then
                                 ". " + n.Institution.Value.Value
                             else
                                 "")
                    | DarkDataSource n ->
                        sprintf
                            "%s unpublished. [dataset] %s%s"
                            (FieldDataTypes.Author.authorList (
                                List.concat [ [ n.Investigator ]; n.AdditionalInvestigators ]
                            ))
                            (if n.Title.IsSome then n.Title.Value.Value + ". " else "")
    ...
    let private takeAlphaNumFirstChars seq =
        seq
        |> Seq.map (Seq.head >> tryAlphanum)
        |> Seq.choose id
        |> Seq.map string
        |> String.concat ""
    ...
    let makeUniqueKey (nodeData: Node) : Graph.UniqueKey =
        let friendlyKey t =
            Graph.FriendlyKey(nodeData.NodeType() |> toLower, t)

        let guidKey g =
            Graph.UUID(nodeData.NodeType() |> toLower, g)
        ...
        | PopulationNode p ->
            ...
            | BioticProxyNode n ->
                match n with
                | Population.BioticProxies.BioticProxyNode.AncientDNA a ->
                    sprintf "aDNA_%s" (safeString a.Value) |> toLower |> friendlyKey
                | Population.BioticProxies.BioticProxyNode.ContemporaneousWholeOrganism taxon ->
                    sprintf "direct_%s" (safeString taxon.Value) |> toLower |> friendlyKey
                | Population.BioticProxies.BioticProxyNode.Morphotype m ->
                    match m with
                    | Population.BioticProxies.Megafossil(part, f) ->
                        sprintf "morphotype_megafossil_%s_%s" (safeString f.Value) (safeString part.Value)
                        |> toLower
                        |> friendlyKey
                    | Population.BioticProxies.Macrofossil(part, f) ->
                        sprintf "morphotype_macrofossil_%s_%s" (safeString f.Value) (safeString part.Value)
                        |> toLower
                        |> friendlyKey
                    | Population.BioticProxies.Microfossil(group, name) ->
                        match group with
                        | Population.BioticProxies.MicrofossilGroup.Diatom ->
                            sprintf "morphotype_diatom_%s" (safeString name.Value) |> toLower |> friendlyKey
                        | Population.BioticProxies.MicrofossilGroup.Ostracod ->
                            sprintf "morphotype_ostracod_%s" (safeString name.Value)
                            |> toLower
                            |> friendlyKey
                        | Population.BioticProxies.MicrofossilGroup.PlantMacrofossil ->
                            sprintf "morphotype_plantmacrofossil_%s" (safeString name.Value)
                            |> toLower
                            |> friendlyKey
                        | Population.BioticProxies.MicrofossilGroup.Pollen ->
                            sprintf "morphotype_pollen_%s" (safeString name.Value) |> toLower |> friendlyKey
                        | Population.BioticProxies.MicrofossilGroup.OtherMicrofossilGroup group ->
                            sprintf "morphotype_customgroup_%s_%s" (safeString group.Value) (safeString name.Value)
                            |> toLower
                            |> friendlyKey
            | BioticProxyCategoryNode n ->
                match n with
                | BioticProxies.BioticProxyCategoryNode.AncientDNA g ->
                    sprintf "aDNA_%s" (safeString <| g.ToString()) |> toLower |> friendlyKey
                | BioticProxies.BioticProxyCategoryNode.Contemporary g ->
                    sprintf "contemporary_%s" (safeString <| g.ToString()) |> toLower |> friendlyKey
                | BioticProxies.BioticProxyCategoryNode.Fossil g ->
                    sprintf "fossil_%s" (safeString <| g.ToString()) |> toLower |> friendlyKey
                | BioticProxies.BioticProxyCategoryNode.OtherProxy p ->
                    sprintf "fossil_%s" (safeString <| p.Value) |> toLower |> friendlyKey
                | BioticProxies.BioticProxyCategoryNode.Microfossil m ->
                    ...
                    | BioticProxies.MicrofossilGroup.PlantMacrofossil ->
                        "fossilmicro_plantmacrofossil" |> toLower |> friendlyKey
                    | BioticProxies.MicrofossilGroup.Pollen -> "fossilmicro_pollen" |> toLower |> friendlyKey
                    | BioticProxies.MicrofossilGroup.OtherMicrofossilGroup group ->
                        sprintf "fossilmicro_other_%s" (safeString group.Value)
                        |> toLower
                        |> friendlyKey
            | TaxonomyNode n ->
                match n with
                ...
                | Taxonomy.TaxonNode.Subfamily l ->
                    sprintf "subfamily_%s" (safeString l.Value) |> toLower |> friendlyKey
                ...
                | Taxonomy.TaxonNode.Species(l, l2, l3) ->
                    sprintf "species_%s_%s_%s" (safeString l.Value) (safeString l2.Value) (safeString (l3.Value))
                    |> toLower
                    |> friendlyKey
                | Taxonomy.TaxonNode.Subspecies(l, l2, l3, l4) ->
                    sprintf
                        "subspecies_%s_%s_%s_%s"
                        (safeString l.Value)
                        (safeString l2.Value)
                        (safeString l3.Value)
                        (System.Net.WebUtility.HtmlEncode(l4.Value))
                    |> toLower
                    |> friendlyKey
                | Taxonomy.TaxonNode.Variety(l, l2, l3, l4) ->
                    sprintf
            ...
            | InferenceMethodNode n ->
                ...
                | BioticProxies.InferenceMethodNode.IdentificationKeyOrAtlas r ->
                    ...
                    |> toLower
                    |> friendlyKey
                | BioticProxies.InferenceMethodNode.ImplicitByExpert(l, i) ->
                    sprintf "expert_%s_%s" (safeString l.Value) (safeString i.Value)
                    |> toLower
                    |> friendlyKey
            | ProxiedTaxonNode -> guidKey (System.Guid.NewGuid())
            | ContextNode _ -> guidKey (System.Guid.NewGuid())
            | VernacularTaxonLabelNode v ->
                sprintf "%s_%s" (safeString v.Language.Value) (safeString v.Label.Value)
                |> toLower
                |> friendlyKey
        | SourceNode s ->
            match s with
            ...
            | Excluded(s, _, _) ->
                ...
                | Bibliographic n ->
                    String.concat
                        "_"
                        [ "pub"
                          (if n.Author.IsSome then
                               n.Author.Value.Value.Split(",").[0]
                           else
                               "unknown")
                          (if n.Title.IsSome then
                               (n.Title.Value.Value.Split(" ")
                                |> Seq.map (Seq.head >> tryAlphanum)
                                |> Seq.choose id
                                |> Seq.map string
                                |> String.concat "")
                ...
                | GreyLiterature n ->
                    sprintf
                        ...
                        (n.Contact.FirstName.Value.Split(" ")
                ...
                | DarkData n ->
                    sprintf
                        "darkdata_%s_%s_%s"
                        (safeString n.Contact.LastName.Value)
                        (safeString n.Contact.FirstName.Value)
                        ((n.Details.Value.Split(" ")
                          |> Seq.map (Seq.head >> tryAlphanum)
                          |> Seq.choose id
                          |> Seq.map string
                          |> Seq.truncate 40
                    ...
                    |> friendlyKey
                | Database n ->
                    sprintf "database_%s" (safeString n.Abbreviation.Value)
                    |> toLower
                    |> friendlyKey
                | DatabaseEntry n ->
                    sprintf
                        "database_%s_entry_%s"
                        (safeString n.DatabaseAbbreviation.Value)
                        (safeString n.UniqueIdentifierInDatabase.Value)
                    |> toLower
                    |> friendlyKey
                | PublishedSource n ->
                    match n with
                    | PublishedSource.Book n ->
                        String.concat
                            ...
                            [ "book"
                              ...
                              (n.BookAdditionalAuthors
                               |> List.map (fun c -> c.Value.LastName)
                               |> takeAlphaNumFirstChars)
                    ...
                    | PublishedSource.IndividualDataset n ->
                        String.concat
                            "_"
                            [ "published-dataset"
                              (if n.Contributors.IsEmpty then
                                   "unk"
                               else
                                   n.Contributors.Head.Value.LastName)
                              (if n.Contributors.IsEmpty then
                                   "unk"
                               else
                                   n.Contributors.Head.Value.Initials |> List.map string |> String.concat "")
                              (if n.Contributors.Length > 1 then
                                   n.Contributors.Tail
                                   |> List.map (fun c -> c.Value.LastName)
                                   |> takeAlphaNumFirstChars
                               else
                                   "single")
                              (if n.YearPublished.IsNone then
                                   "unk"
                               else
                                   string n.YearPublished.Value)
                ...
                | GreyLiteratureSource n ->
                    sprintf
                        ...
                        (if n.Contributors.Length > 1 then
                             ...
                             "single")
                        (if n.PostedYear.IsNone then
                             "unk"
                         else
                             string n.PostedYear.Value)
                        (n.Title.Value.Split(" ") |> takeAlphaNumFirstChars)
                    |> toLower
                    |> friendlyKey
                | DarkDataSource n ->
                    sprintf
                        "darkdata_%s_%s_%s_%s_%s"
                        n.Investigator.Value.LastName
                        (n.Investigator.Value.Initials |> List.map string |> String.concat "")
                        (n.AdditionalInvestigators
                         |> List.map (fun c -> c.Value.LastName)
                         |> takeAlphaNumFirstChars)
                        (if n.CompilationYear.IsNone then
                             "unk"
                         else
                             string n.CompilationYear.Value)
        ...
        | ExposureNode e ->
            ...
            | SliceLabelNode n ->
                sprintf "%s_by_%s" (safeString n.Name.Value) (safeString n.DesignatingAuthority.Value)
                |> toLower
                |> friendlyKey
        ...
        | OutcomeNode o ->
            ...
            | MeasureNode n ->
                ...
                | Biodiversity.BiodiversityDimensionNode.OtherBiodiversityDimension o ->
                    sprintf "custom_%s" (safeString o.Value) |> toLower |> friendlyKey
    ...
    module Nodes =
        ...
        let asPopnNode =
            function
            | PopulationNode n -> Some n
            | _ -> None

        let asExposureNode =
            function
            | ExposureNode n -> Some n
            | _ -> None
        ...
        let tryFindTaxon cond graph =
            tryFind asPopnNode whereTaxon' cond graph

        let tryFindProxy cond graph =
            tryFind asPopnNode whereBioticProxy' cond graph

        let tryFindInferenceMethod cond graph =
            tryFind asPopnNode whereInferenceMethod' cond graph

        let tryFindTimePeriod label graph =
            tryFind asExposureNode whereTime' label graph

        let tryFindYear year graph =
            tryFind asExposureNode whereYear' year graph
        ...
        let tryMakeNode t (n: obj) =
            try
                ...
                | (t: System.Type) when t = typeof<Sources.Source> ->
                    n :?> Sources.Source |> Unscreened |> SourceNode |> Ok
                // Population nodes:
                | (t: System.Type) when t = typeof<BioticProxies.BioticProxyNode> ->
                    n :?> BioticProxies.BioticProxyNode |> BioticProxyNode |> PopulationNode |> Ok
                | (t: System.Type) when t = typeof<BioticProxies.BioticProxyCategoryNode> ->
                    n :?> BioticProxies.BioticProxyCategoryNode
                    |> BioticProxyCategoryNode
                    |> PopulationNode
                    |> Ok
                | (t: System.Type) when t = typeof<Taxonomy.TaxonNode> ->
                    n :?> Taxonomy.TaxonNode |> TaxonomyNode |> PopulationNode |> Ok
                | (t: System.Type) when t = typeof<BioticProxies.InferenceMethodNode> ->
                    n :?> BioticProxies.InferenceMethodNode
                    |> InferenceMethodNode
                    |> PopulationNode
                    |> Ok
                | (t: System.Type) when t = typeof<Population.Taxonomy.VernacularTaxonLabelNode> ->
                    n :?> Population.Taxonomy.VernacularTaxonLabelNode
                    |> VernacularTaxonLabelNode
                    |> PopulationNode
                    |> Ok
                | t when t = typeof<Population.Context.ContextNode> ->
                    n :?> Population.Context.ContextNode |> ContextNode |> PopulationNode |> Ok
                // Exposure node:
                | (t: System.Type) when t = typeof<TemporalIndex.CalYearNode> ->
                    n :?> TemporalIndex.CalYearNode |> YearNode |> ExposureNode |> Ok
                | (t: System.Type) when t = typeof<TemporalIndex.QualitativeLabelNode> ->
                    n :?> TemporalIndex.QualitativeLabelNode |> SliceLabelNode |> ExposureNode |> Ok
                | (t: System.Type) when t = typeof<StudyTimeline.IndividualTimelineNode> ->
                    n :?> StudyTimeline.IndividualTimelineNode |> TimelineNode |> ExposureNode |> Ok
                | (t: System.Type) when t = typeof<StudyTimeline.IndividualDateNode> ->
                    n :?> StudyTimeline.IndividualDateNode |> DateNode |> ExposureNode |> Ok
                // Outcome node:
                | (t: System.Type) when t = typeof<Outcomes.Biodiversity.BiodiversityDimensionNode> ->
                    n :?> Biodiversity.BiodiversityDimensionNode |> MeasureNode |> OutcomeNode |> Ok
                | _ -> Error <| sprintf "Not a known node type (%s)" t.Name
            with e ->
                Error <| sprintf "Failed to make a node: %s" e.Message
    ...
    module Relations =
            ...
            let compareTypes (node: System.Guid * 'node) rel paramIndex =
                ...
                | Some(c: UnionCaseInfo) ->
                    ...
                    let nodeType =
                        ...
                        if baseType = typeof<System.Object> then
                            (snd node).GetType()
                        else
                            baseType
        ...
        let makeRelation source sink (rel: ProposedRelation) : Result<ValidRelation<Relation>, string> =
            ...
            | Exposure rel ->
                ...
                | ExtentEarliestOutOfScope t ->
                    compare source sink rel (Relation.Exposure <| ExtentEarliestOutOfScope t)
            ...
            | Source rel ->
                ...
                | UsedDatabase(accessDate, subset) ->
                    compare source sink rel (Relation.Source <| UsedDatabase(accessDate, subset))
        ...
        let addRelation
            (atom1: Graph.Atom<'data, 'conn>)
            (atom2: Graph.Atom<'data2, 'conn2>)
            rel
            weight
            (graph: Graph.Graph<'a, Relation>)
            =
            result {
        ...
        let addProxiedTaxon (edge: ProxiedTaxon.ProxiedTaxonHyperEdge) graph =
            result {
                ...
                let! existingInfer =
                    graph |> Nodes.tryFindInferenceMethod (fun n -> n = edge.InferredUsing), "infer doesn't exist"

                let! proxiedGraph = Graph.addNodeData makeUniqueKey [ PopulationNode ProxiedTaxonNode ] graph

                let! proxiedTaxon =
                    Graph.getAtom (snd proxiedGraph |> List.last |> fst) (fst proxiedGraph), "no intermediate node"
        ...
        let nodeIdsByRelation<'a> (relationCase: obj) (atom: Graph.Atom<Node, Relation>) =
            atom
            |> snd
            |> Seq.choose (fun (source, sink, _, r) ->
                match r with
                | Relation.Exposure e ->
                    if typeof<'a> = typeof<ExposureRelation> then
                        if e = (relationCase :?> ExposureRelation) then
                            Some sink
                        else
                            None
                    else
                ...
                | Relation.Source s ->
                    if typeof<'a> = typeof<SourceRelation> then
                        if s = (relationCase :?> SourceRelation) then
                            Some sink
                        else
                            None
                    else
                ...
                | Relation.Population p ->
                    if typeof<'a> = typeof<PopulationRelation> then
                        if p = (relationCase :?> PopulationRelation) then
                            Some sink
                        else
                            None
                    else
