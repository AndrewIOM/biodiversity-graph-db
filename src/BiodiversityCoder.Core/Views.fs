...
module ViewParts =
    ...
    module Grid =
        ...
        let row (node:Node) = div { 
            _class "row"
            node }
        
        let column n (node:Node) = div {
    ...
    let alertMessage' level msg dismissAction =
        concat {
            alert level msg
            button {
                _class "btn"; on.click (fun _ -> dismissAction)
                text "Dismiss"
            }
        }

    let alertMessage message dismissAction =
        cond message <| function
        | Some msg ->
...
module AtomView =
    ...
    let timeline timelineAtom g =
        ViewParts.card None ViewParts.CardLevel.Secondary <|
            concat {
                cond (Storage.tryFindRelationFriendlyName<Exposure.ExposureRelation> Exposure.ExposureRelation.ExtentEarliest g timelineAtom) <| function
                | Some early ->
                    cond (Storage.tryFindRelationFriendlyName<Exposure.ExposureRelation> Exposure.ExposureRelation.ExtentLatest g timelineAtom) <| function
                    | Some late ->
                        h5 {
                            _class "card-title"
                ...
                | Some exposureNode ->
                        ...
                        | Exposure.StudyTimeline.Continuous res ->
                            ...
                            | Exposure.StudyTimeline.Regular (r,g) ->
                                h6 {
                                    ...
                                    textf "A continuous timeline with %A year timesteps (formed using %A)" r g
                                }
                    ...
                    | _ -> empty ()
                | _ -> empty ()
                cond (Storage.tryFindRelationNode<Exposure.ExposureRelation> Exposure.ExposureRelation.IsLocatedAt g timelineAtom) <| function
                | Some context ->
                    p {
                ...
                | None -> empty ()
                small {
                    text "The time-series is constructed from the following individual dates:"
                }
                ul {
                    forEach (Storage.tryFindRelationNodes<Exposure.ExposureRelation> Exposure.ExposureRelation.ConstructedWithDate g timelineAtom) <| fun dateNode ->
                        cond (dateNode |> fst |> snd) <| function
                        | GraphStructure.Node.ExposureNode p ->
                            cond p <| function
                            | Exposure.ExposureNode.DateNode d ->
                                li {
                                    textf "Esimated date from %A (depth %A). Method was %A." d.MaterialDated.Value d.SampleDepth d.Date
                                }
                            | _ -> empty ()
                        | _ -> empty ()
                }
