...
module FieldDataTypes =
    ...
    module Uri =
        ...
        type System.Uri with
            static member TryCreate s =
                match s with
                | SimpleValue.Text s ->
                    if System.Uri.CheckSchemeName(s) then
                        Uri(s) |> Some
                    else
                        None
                | _ -> None
    ...
    module Text =
        ...
        type ShortText with
            member this.Value = this |> (fun (ShortText t) -> t)

        type Text with
            member this.Value = this |> (fun (Text t) -> t)

        let createShort txt =
            if not (String.IsNullOrEmpty txt) then
                if txt.Length > 100 then
                    Error "Short text must be less than 100 characters"
                else
                    Ok(ShortText txt)
            else
                Error "Short text must not be empty"
        ...
        type ShortText with
            static member TryCreate s =
                match s with
                | SimpleValue.Text s -> createShort s |> Result.toOption
                | _ -> None

        type Text with
            static member TryCreate s =
                match s with
                | SimpleValue.Text s -> create s |> Result.toOption
                | _ -> None
    ...
    module Author =
        ...
        let regex =
            "(\S+) ?([A-z]*), ?([A-z]){1}[\.| ]{0,2}([A-z]){0,1}[\.| ]{0,2}([A-z]){0,1}"
        ...
        let create (authorString: string) =
            if Text.RegularExpressions.Regex.IsMatch(authorString, regex) then
                ...
                let initials =
                    if m.Groups.[5].Success then
                        [ m.Groups.[3].Value.[0]; m.Groups.[4].Value.[0]; m.Groups.[5].Value.[0] ]
                    else if m.Groups.[4].Success then
                        [ m.Groups.[3].Value.[0]; m.Groups.[4].Value.[0] ]
                    else if m.Groups.[3].Success then
                        [ m.Groups.[3].Value.[0] ]
                    else
                        []
                ...
                Error "Authors must be specified in the format: Surname, M. A. C. (or Surname,M.A.C or Surname, M A C)"
        ...
        let authorList authors =
            if authors |> Seq.isEmpty then
                "Unknown author(s)"
            else
                authors |> List.map (fun (d: Author) -> d.Display) |> String.concat "; "
        ...
        let authorListTruncated (authors: Author list) nDisplay =
            if authors |> Seq.isEmpty then
                "Unknown author(s)"
            else if authors.Length > nDisplay then
                (authors
                 |> List.truncate nDisplay
                 |> List.map (fun (d: Author) -> d.Display)
                 |> String.concat "; ")
                + " et al."
            else
                authors |> List.map (fun (d: Author) -> d.Display) |> String.concat "; "
    ...
    module DigitalObjectIdentifier =
        ...
        let create doi =
            if Text.RegularExpressions.Regex.IsMatch(doi, doiRegex) then
                let m = Text.RegularExpressions.Regex.Match(doi, doiRegex)
                m.Groups.[1].Value |> DigitalObjectIdentifier |> Ok
            else
                Error "DOIs must be in the format 10.1126/science.aar3646 or https://doi.org/10.1126/science.aar3646"
    ...
    module IntRange =
        ...
        let create doi =
            if Text.RegularExpressions.Regex.IsMatch(doi, regex) then
                ...
                Error "Ranges should be in the format 12 - 14"
    ...
    module Geography =
        ...
        let createLatitude lat =
            if lat >= -89.9 && lat <= 89.9 then
                lat * 1.<DD> |> Latitude |> Ok
            else
                Error "Latitude must be between -90 and 90 degrees"

        let createLongitude lon =
            if lon >= -180. && lon <= 180. then
                lon * 1.<DD> |> Longitude |> Ok
            else
        ...
        let createCoordinate coordString =
            if
                System.Text.RegularExpressions.Regex.IsMatch(
                    coordString,
                    "^([0-9]{1,2})[:|°]([0-9]{1,2})[:|'|′]?([0-9]{1,2}(?:\.[0-9]+){0,1})?[\"|″]([N|S]),([0-9]{1,3})[:|°]([0-9]{1,2})[:|'|′]?([0-9]{1,2}(?:\.[0-9]+){0,1})?[\"|″]([E|W])$"
                )
            then
                coordString |> CoordinateDMS |> Ok
            else
                Error "Coordinate was not in the format: 40°26'46\"N,79°01'00\"W"
        ...
        type SamplingLocation =
            ...
            | Locality of
                locality: Text.ShortText *
                district: Text.ShortText *
                region: Text.ShortText *
            ...
            static member TryCreate s =
                ...
                | Text s ->
                    ...
                    if m.Success then
                        ...
                        if Array.TrueForAll(coords, (fun f -> f.Length = 2)) then
                            coords
                            |> Array.map (fun c ->
                            ...
                            |> Result.bind (fun l ->
                                if l.Length < 3 then
                                    Error "Cannot have less than three points for a polygon"
                                else
                                    Ok <| Polygon l)
                        else
                            Error "Coordinates must be defined in format 'lon lat, lon lat, lon lat'"
                    else
                        Error "You must enter a polygon as a WKT format string, starting with POLYGON."
    ...
    type Person =
        { FirstName: Text.ShortText
          LastName: Text.ShortText }
